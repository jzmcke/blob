...
extern "C"
{
    ...
    extern int _blob_espudp_init(blob_comm_cfg*, int, int, int, int, int, int);
    extern int _blob_espudp_terminate(blob_comm_cfg*);
    #define BLOB_INIT(addr0, addr1, addr2, addr3, port, latency)       _blob_espudp_init(&g_blob_ccfg, addr0, addr1, addr2, addr3, port, latency); blob_init(&g_blob_ccfg)
    #define BLOB_TERMINATE()                                           _blob_espudp_terminate(&g_blob_ccfg)
#endif
